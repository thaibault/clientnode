...
qunit.test 'normalizeClassNames', ->
    ...
    qunit.strictEqual(
        ...
        $('<div class="a b"><pre class="a b c"></pre></div>').prop 'outerHTML')
...
qunit.test 'isolateScope', ->
    ...
    qunit.deepEqual tools.isolateScope(a: 2, b: a: [1, 2]), a: 2, b: a: [1, 2]
    scope = -> this.a = 2
    ...
    qunit.deepEqual tools.isolateScope(scope, []), _a: undefined, a: 2, b: 3
    scope._a = 6
...
qunit.test 'equals', ->
    ...
    qunit.ok tools.equals (new window.Date), (new window.Date)
    qunit.ok tools.equals (new window.Date 1995, 11, 17), (new window.Date(
        1995, 11, 17))
    qunit.notOk tools.equals (new window.Date 1995, 11, 17), (new window.Date(
        1995, 11, 16))
...
qunit.test 'arrayAggregatePropertyIfEqual', ->
    qunit.strictEqual tools.arrayAggregatePropertyIfEqual([a: 'b'], 'a'), 'b'
...
qunit.test 'arrayExtractIfMatches', ->
    ...
    qunit.deepEqual tools.arrayExtractIfMatches(['a', 'b'], '[ab]'), ['a', 'b']
...
qunit.test 'stringGetPortNumber', ->
    ...
    qunit.strictEqual tools.stringGetPortNumber(), window.parseInt(
        window.location.port)
qunit.test 'stringGetProtocolName', ->
    qunit.strictEqual tools.stringGetProtocolName(
        'https://www.test.de/site/subSite?param=value#hash'
    ), 'https'
    qunit.strictEqual tools.stringGetProtocolName('http://www.test.de'), 'http'
    qunit.strictEqual tools.stringGetProtocolName(
        '//www.test.de'
...
qunit.test 'stringIsInternalURL', ->
    ...
    qunit.ok tools.stringIsInternalURL(
        ...
        '//www.test.de/site/subSite?param=value#hash')
    qunit.ok tools.stringIsInternalURL window.location.href
