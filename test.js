...
browserAPI((browserAPI:BrowserAPI):void => {
    ...
    for (const roundType:string of ['plain', 'withDocument', 'withJQuery']) {
        ...
        QUnit.module(`tools (${roundType})`)
        // region tests
        // / region public methods
        // // region special
        QUnit.test(`constructor (${roundType})`, (assert:Object):void =>
            assert.ok(tools))
        QUnit.test(`destructor (${roundType})`, (assert:Object):void =>
            assert.strictEqual(tools.destructor(), tools))
        QUnit.test(`initialize (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`controller (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`acquireLock|releaseLock (${roundType})`, (
            assert:Object
        ):void => {
            let testValue = false
        ...
        // // endregion
        // // region boolean
        QUnit.test(`isNumeric (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`isWindow (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`isArrayLike (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`isAnyMatching (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                ['', ['']],
                ['test', [/test/]],
                ['test', [/a/, /b/, /es/]],
        ...
        QUnit.test(`isPlainObject (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`isFunction (${roundType})`, (assert:Object):void => {
            for (const okValue:any of [
        ...
        if (roundType === 'withJQuery') {
            QUnit.test(`getText (${roundType})`, (assert:Object):void => {
            ...
            QUnit.test(`normalizeClassNames (${roundType})`, (
                assert:Object
            ):void => {
            ...
            QUnit.test(`normalizeStyles (${roundType})`, (
                assert:Object
            ):void => {
                assert.strictEqual($('<div>').Tools(
                    'normalizeStyles'
            ...
            QUnit.test(`isEquivalentDom (${roundType})`, (
                assert:Object
            ):void => {
                for (const test:Array<any> of [
                    ['test', 'test'],
        ...
        if (roundType === 'withJQuery')
            QUnit.test(`getPositionRelativeToViewport (${roundType})`, (
                assert:Object
            ):void => assert.ok([
                'above', 'left', 'right', 'below', 'in'
            ].includes(tools.getPositionRelativeToViewport())))
        QUnit.test(`generateDirectiveSelector (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<string> of [
                [
        ...
        if (roundType === 'withJQuery')
            QUnit.test(`removeDirective (${roundType})`, (
                assert:Object
            ):void => {
        ...
        QUnit.test(`getNormalizedDirectiveName (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<string> of [
                ['data-a', 'a'],
        ...
        if (roundType === 'withJQuery')
            QUnit.test(`getDirectiveValue (${roundType})`, (
                assert:Object
            ):void => assert.equal(
                $('body').Tools('getDirectiveValue', 'a'), null))
        QUnit.test(`sliceDomNodeSelectorPrefix (${roundType})`, (
            assert:Object
        ):void => {
            assert.strictEqual(
        ...
        QUnit.test(`getDomNodeName (${roundType})`, (assert:Object):void => {
            for (const test:Array<string> of [
                ['div', 'div'],
                ['<div>', 'div'],
                ['<div />', 'div'],
        ...
        if (roundType === 'withJQuery')
            QUnit.test(`grabDomNode (${roundType})`, (assert:Object):void => {
                for (const test:Array<any> of [
                    [
                        [{
        ...
        // // endregion
        // // region scope
        QUnit.test(`isolateScope (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`determineUniqueScopeName (${roundType})`, (
            assert:Object
        ):void => {
        ...
        QUnit.test(`getMethod (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`identity (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [2, 2],
                ['', ''],
                [undefined, undefined],
        ...
        QUnit.test(`invertArrayFilter (${roundType})`, (assert:Object):void => {
        ...
        // // endregion
        // // region event
        QUnit.test(`debounce (${roundType})`, (assert:Object):void => {
            let testValue = false
        ...
        QUnit.test(`fireEvent (${roundType})`, (assert:Object):void => {
            let testValue = false
        ...
        if (roundType === 'withJQuery') {
            QUnit.test(`on (${roundType})`, (assert:Object):void => {
                let testValue = false
            ...
            QUnit.test(`off (${roundType})`, (assert:Object):void => {
                let testValue = false
                assert.strictEqual(tools.on('body', 'click', ():void => {
                    testValue = true
                })[0], $('body')[0])
        ...
        // // endregion
        // // region object
        QUnit.test(`determineType (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`convertSubstringInPlainObject (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [{}, /a/, '', {}],
        ...
        QUnit.test(`extendObject (${roundType})`, (assert:Object):void => {
            for (const test:any of [
                [[[]], []],
                [[{}], {}],
                [[{a: 1}], {a: 1}],
        ...
        QUnit.test(`unwrapProxy (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [{}, {}],
                [{a: 'a'}, {a: 'a'}],
                [{a: 'aa'}, {a: 'aa'}],
        ...
        QUnit.test(`addDynamicGetterAndSetter (${roundType})`, (
            assert:Object
        ):void => {
            assert.strictEqual(
        ...
        QUnit.test(`resolveDynamicDataStructure (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[null], null],
        ...
        QUnit.test(`convertPlainObjectToMap (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[null], null],
        ...
        QUnit.test(`convertMapToPlainObject (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[null], null],
        ...
        QUnit.test(`forEachSorted (${roundType})`, (assert:Object):void => {
            let result = []
        ...
        QUnit.test(`sort (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[], []],
                [{}, []],
                [[1], [0]],
        ...
        QUnit.test(`equals (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [1, 1],
                [new Date(), new Date()],
        ...
        QUnit.test(`copyLimitedRecursively (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[21], 21],
        ...
        // // endregion
        // // region array
        QUnit.test(`arrayMerge (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[], [], []],
                [[1], [], [1]],
                [[], [1], [1]],
        ...
        QUnit.test(`arrayMake (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[], []],
                [[1, 2, 3], [1, 2, 3]],
                [1, [1]]
        ...
        QUnit.test(`arrayUnique (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[1, 2, 3, 1], [1, 2, 3]],
                [[1, 2, 3, 1, 2, 3], [1, 2, 3]],
                [[], []],
        ...
        QUnit.test(`arrayAggregatePropertyIfEqual (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[[{a: 'b'}], 'a'], 'b'],
        ...
        QUnit.test(`arrayDeleteEmptyItems (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[[{a: null}]], []],
        ...
        QUnit.test(`arrayExtract (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[[{a: 'b', c: 'd'}], ['a']], [{a: 'b'}]],
                [[[{a: 'b', c: 'd'}], ['b']], [{}]],
                [[[{a: 'b', c: 'd'}], ['c']], [{c: 'd'}]],
        ...
        QUnit.test(`arrayExtractIfMatches (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [['b'], /b/, ['b']],
        ...
        QUnit.test(`arrayExtractIfPropertyExists (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[{a: 2}], 'a', [{a: 2}]],
        ...
        QUnit.test(`arrayExtractIfPropertyMatches (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[{a: 'b'}], {a: 'b'}, [{a: 'b'}]],
        ...
        QUnit.test(`arrayIntersect (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[['A'], ['A']], ['A']],
                [[['A', 'B'], ['A']], ['A']],
                [[[], []], []],
        ...
        QUnit.test(`arrayMakeRange (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[[0]], [0]],
                [[[5]], [0, 1, 2, 3, 4, 5]],
                [[[]], []],
        ...
        QUnit.test(`arraySumUpProperty (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[[{a: 2}, {a: 3}], 'a'], 5],
        ...
        QUnit.test(`arrayAppendAdd (${roundType})`, (assert:Object):void => {
        ...
        QUnit.test(`arrayRemove (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[[], 2], []],
                [[[2], 2], []],
                [[[2], 2, true], []],
        ...
        QUnit.test(`stringAddSeparatorToPath (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [[''], ''],
        ...
        QUnit.test(`stringHasPathPrefix (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                ['/admin', '/admin'],
        ...
        QUnit.test(`stringGetDomainName (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [
        ...
        QUnit.test(`stringGetPortNumber (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
        ...
        QUnit.test(`stringGetProtocolName (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [
        ...
        QUnit.test(`stringGetURLVariable (${roundType})`, (
            assert:Object
        ):void => {
        ...
        QUnit.test(`stringIsInternalURL (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [
        ...
        QUnit.test(`stringNormalizeURL (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
        ...
        QUnit.test(`stringRepresentURL (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
        ...
        // /// endregion
        QUnit.test(`stringCompressStyleValue (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                ['', ''],
        ...
        QUnit.test(`stringCamelCaseToDelimited (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
        ...
        QUnit.test(`stringCapitalize (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                ['hansPeter', 'HansPeter'],
                ['', ''],
                ['a', 'A'],
        ...
        QUnit.test(`stringDelimitedToCamelCase (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
        ...
        QUnit.test(`stringFormat (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [['{1}', 'test'], 'test'],
                [['', 'test'], ''],
                [['{1}'], '{1}'],
        ...
        QUnit.test(`stringGetRegularExpressionValidated (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                ["that's no regex: .*$", "that's no regex: \\.\\*\\$"],
        ...
        QUnit.test(`stringLowerCase (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                ['HansPeter', 'hansPeter'],
                ['', ''],
                ['A', 'a'],
        ...
        QUnit.test(`stringMark (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[''], ''],
        ...
        QUnit.test(`stringMD5 (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
        ...
        QUnit.test(`stringNormalizePhoneNumber (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                ['0', '0'],
        ...
        QUnit.test(`stringRepresentPhoneNumber (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                ['0', '0'],
        ...
        QUnit.test(`stringDecodeHTMLEntities (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<string> of [
                ['', ''],
        ...
        QUnit.test(`normalizeDomNodeSelector (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<string> of [
                ['div', 'body div'],
        ...
        // / endregion
        // // region number
        QUnit.test(`numberIsNotANumber (${roundType})`, (
            assert:Object
        ):void => {
            for (const test:Array<any> of [
                [NaN, true],
        ...
        QUnit.test(`numberRound (${roundType})`, (assert:Object):void => {
            for (const test:Array<any> of [
                [[1.5, 0], 2],
                [[1.4, 0], 1],
                [[1.4, -1], 0],
        ...
        if (
            ...
            QUnit.test(`sendToExternalURL (${roundType})`, (
                assert:Object
        ...
        // / region protected
        if (roundType === 'withJQuery')
            QUnit.test(`_bindEventHelper (${roundType})`, (
                assert:Object
            ):void => {
                for (const test:Array<any> of [
                    [['body']],
